...
func TestVersions(t *testing.T) {
	...
	for _, c := range []vfpair{
		...
		{pkgbits.V3, pkgbits.AliasTypeParamNames},
		{pkgbits.V3, pkgbits.FuncProtection},
	} {
		if !c.v.Has(c.f) {
	...
	for _, c := range []vfpair{
		...
		{pkgbits.V3, pkgbits.HasInit},
		{pkgbits.V2, pkgbits.DerivedFuncInstance},
		{pkgbits.V3, pkgbits.DerivedFuncInstance},
		{pkgbits.V2, pkgbits.DerivedInfoNeeded},
		{pkgbits.V3, pkgbits.DerivedInfoNeeded},
		{pkgbits.V0, pkgbits.AliasTypeParamNames},
		{pkgbits.V1, pkgbits.AliasTypeParamNames},
		{pkgbits.V0, pkgbits.FuncProtection},
		{pkgbits.V1, pkgbits.FuncProtection},
		{pkgbits.V2, pkgbits.FuncProtection},
	} {
		if c.v.Has(c.f) {
