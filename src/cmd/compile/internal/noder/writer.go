...
func newPkgWriter(m posMap, pkg *types2.Package, info *types2.Info, otherInfo map[*syntax.FuncLit]bool) *pkgWriter {
	// Use V3 for per-function protection metadata.
	version := pkgbits.V3
	return &pkgWriter{
...
func (w *writer) funcExt(obj *types2.Func) {
	...
	protection := asProtectionFlag(decl.Pragma)
	if pragma&ir.Systemstack != 0 && pragma&ir.Nosplit != 0 {
		w.p.errorf(decl, "go:nosplit and go:systemstack cannot be combined")
	}
	const activeProtection = ir.ProtectObfuscate | ir.ProtectEncrypt | ir.ProtectVirtualize | ir.ProtectEphemeral | ir.ProtectStream | ir.ProtectStreamV5 | ir.ProtectStreamV6
	...
	if protection&ir.ProtectStream != 0 && protection&ir.ProtectEncrypt == 0 {
		w.p.errorf(decl, "go:stream requires go:encrypt")
	}
	if protection&ir.ProtectStream != 0 && protection&ir.ProtectVirtualize != 0 {
		w.p.errorf(decl, "go:stream cannot be combined with go:vm")
	}
	if protection&ir.ProtectStream != 0 && protection&ir.ProtectEphemeral != 0 {
		w.p.errorf(decl, "go:stream cannot be combined with go:ephemeral")
	}
	if protection&ir.ProtectStreamV5 != 0 && protection&ir.ProtectEncrypt == 0 {
		w.p.errorf(decl, "go:streamv5 requires go:encrypt")
	}
	if protection&ir.ProtectStreamV5 != 0 && protection&ir.ProtectVirtualize != 0 {
		w.p.errorf(decl, "go:streamv5 cannot be combined with go:vm")
	}
	if protection&ir.ProtectStreamV5 != 0 && protection&ir.ProtectEphemeral != 0 {
		w.p.errorf(decl, "go:streamv5 cannot be combined with go:ephemeral")
	}
	if protection&ir.ProtectStreamV5 != 0 && protection&ir.ProtectStream != 0 {
		w.p.errorf(decl, "go:streamv5 cannot be combined with go:stream")
	}
	if protection&ir.ProtectStreamV6 != 0 && protection&ir.ProtectEncrypt == 0 {
		w.p.errorf(decl, "go:streamv6 requires go:encrypt")
	}
	if protection&ir.ProtectStreamV6 != 0 && protection&ir.ProtectVirtualize != 0 {
		w.p.errorf(decl, "go:streamv6 cannot be combined with go:vm")
	}
	if protection&ir.ProtectStreamV6 != 0 && protection&ir.ProtectEphemeral != 0 {
		w.p.errorf(decl, "go:streamv6 cannot be combined with go:ephemeral")
	}
	if protection&ir.ProtectStreamV6 != 0 && protection&(ir.ProtectStream|ir.ProtectStreamV5) != 0 {
		w.p.errorf(decl, "go:streamv6 cannot be combined with another stream directive")
	}
	if protection&(ir.ProtectEphemeral|ir.ProtectStream|ir.ProtectStreamV5|ir.ProtectStreamV6) != 0 {
		sig := obj.Type().(*types2.Signature)
		for i := 0; i < sig.Results().Len(); i++ {
			basic, ok := sig.Results().At(i).Type().Underlying().(*types2.Basic)
			if ok && basic.Info()&types2.IsString != 0 {
				if protection&ir.ProtectStreamV6 != 0 {
					w.p.errorf(decl, "go:streamv6 functions cannot return string values")
				} else if protection&ir.ProtectStreamV5 != 0 {
					w.p.errorf(decl, "go:streamv5 functions cannot return string values")
				} else if protection&ir.ProtectStream != 0 {
					w.p.errorf(decl, "go:stream functions cannot return string values")
				} else {
	...
	w.pragmaFlag(pragma)
	if w.Version().Has(pkgbits.FuncProtection) {
		w.Int(int(protection))
	}
	w.linkname(obj)
...
func (c *declCollector) Visit(n syntax.Node) syntax.Visitor {
	...
	switch n := n.(type) {
	case *syntax.File:
		pw.checkPragmas(n.Pragma, ir.GoBuildPragma, false, false)

	case *syntax.ImportDecl:
		pw.checkPragmas(n.Pragma, 0, false, false)
	...
	case *syntax.FuncDecl:
		pw.checkPragmas(n.Pragma, funcPragmas, false, true)
	...
	case *syntax.VarDecl:
		pw.checkPragmas(n.Pragma, 0, true, false)
...
func (pw *pkgWriter) checkPragmas(p syntax.Pragma, allowed ir.PragmaFlag, embedOK, protectionOK bool) {
	if p == nil {
		return
	}
	pragma := p.(*pragmas)
	...
	if !protectionOK {
		for _, pos := range pragma.ProtectionPos {
			pw.errorf(pos, "misplaced compiler directive")
		}
	}
...
func asProtectionFlag(p syntax.Pragma) ir.ProtectionFlag {
	if p == nil {
		return 0
	}
	return p.(*pragmas).Protection
}
