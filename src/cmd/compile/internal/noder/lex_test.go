...
func TestProtectionPragmaFlags(t *testing.T) {
	tests := []struct {
		verb string
		want ir.ProtectionFlag
	}{
		{"go:obf", ir.ProtectObfuscate},
		{"go:encrypt", ir.ProtectEncrypt},
		{"go:vm", ir.ProtectVirtualize},
		{"go:noprotect", ir.ProtectExclude},
		{"go:ephemeral", ir.ProtectEphemeral},
		{"go:stream", ir.ProtectStream},
		{"go:streamv5", ir.ProtectStreamV5},
		{"go:streamv6", ir.ProtectStreamV6},
	}
	for _, test := range tests {
		if got := protectionFlag(test.verb); got != test.want {
			t.Errorf("protectionFlag(%q) = %#x; want %#x", test.verb, got, test.want)
		}
		if got := pragmaFlag(test.verb); got != 0 {
			t.Errorf("pragmaFlag(%q) = %#x; want 0", test.verb, got)
		}
	}
}
