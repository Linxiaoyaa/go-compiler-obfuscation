...
type pragmas struct {
	Flag          ir.PragmaFlag     // collected standard compiler bits
	Protection    ir.ProtectionFlag // collected protection compiler bits
	Pos           []pragmaPos       // position of each individual standard flag
	ProtectionPos []syntax.Pos      // position of each individual protection flag
	Embeds        []pragmaEmbed
...
func (p *noder) pragma(pos syntax.Pos, blankLine bool, text string, old syntax.Pragma) syntax.Pragma {
	...
	default:
		...
		protection := protectionFlag(verb)
		...
		if flag == 0 && protection == 0 && !allowedStdPragmas[verb] && base.Flag.Std {
			p.error(syntax.Error{Pos: pos, Msg: fmt.Sprintf("//%s is not allowed in the standard library", verb)})
		}
		if protection != 0 {
			pragma.Protection |= protection
			pragma.ProtectionPos = append(pragma.ProtectionPos, pos)
		} else {
			pragma.Flag |= flag
